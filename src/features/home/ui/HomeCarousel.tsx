import type { ReactNode } from "react";
import {
  Carousel,
  CarouselContent,
  CarouselItem,
  CarouselNext,
  CarouselPrevious,
} from "@/components";

type SlideData = {
  label: string;
  title: string;
  description: string;
  gradient: string;
  iconPaths: ReactNode;
};

const SLIDES: SlideData[] = [
  {
    label: "NovaWiki",
    title: "지식의 바다에 오신 것을 환영합니다",
    description: "누구나 참여하고, 함께 만들어가는 백과사전",
    gradient: "from-slate-900 to-slate-700",
    iconPaths: (
      <>
        <path d="M4 19.5v-15A2.5 2.5 0 0 1 6.5 2H20v20H6.5a2.5 2.5 0 0 1 0-5H20" />
        <path d="M8 7h6" />
        <path d="M8 11h8" />
      </>
    ),
  },
  {
    label: "Contribute",
    title: "문서를 작성해보세요",
    description: "당신의 지식이 누군가에게 큰 도움이 됩니다",
    gradient: "from-blue-600 to-indigo-800",
    iconPaths: (
      <>
        <path d="M12 20h9" />
        <path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z" />
      </>
    ),
  },
  {
    label: "Community",
    title: "함께 만드는 커뮤니티",
    description: "토론하고, 수정하고, 더 나은 정보를 완성하세요",
    gradient: "from-emerald-600 to-teal-800",
    iconPaths: (
      <>
        <path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2" />
        <circle cx="9" cy="7" r="4" />
        <path d="M22 21v-2a4 4 0 0 0-3-3.87" />
        <path d="M16 3.13a4 4 0 0 1 0 7.75" />
      </>
    ),
  },
  {
    label: "Explore",
    title: "궁금한 것을 검색해보세요",
    description: "수천 개의 문서가 당신을 기다리고 있습니다",
    gradient: "from-amber-500 to-orange-700",
    iconPaths: (
      <>
        <circle cx="11" cy="11" r="8" />
        <path d="m21 21-4.3-4.3" />
      </>
    ),
  },
  {
    label: "New",
    title: "새로운 기능이 추가되었습니다",
    description: "문서 비교, 편집 히스토리를 확인해보세요",
    gradient: "from-violet-600 to-purple-900",
    iconPaths: (
      <>
        <path d="M12 2v4" />
        <path d="m6.34 6.34 2.83 2.83" />
        <path d="M2 12h4" />
        <path d="m17.66 6.34-2.83 2.83" />
        <path d="M22 12h-4" />
        <path d="M12 22v-4" />
        <path d="m6.34 17.66 2.83-2.83" />
        <path d="m17.66 17.66-2.83-2.83" />
      </>
    ),
  },
];

function SlideIcon({
  title,
  children,
}: {
  title: string;
  children: ReactNode;
}) {
  return (
    <svg
      className="w-30 sm:w-45"
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="1"
      strokeLinecap="round"
      strokeLinejoin="round"
    >
      <title>{title}</title>
      {children}
    </svg>
  );
}

function CarouselSlide({
  label,
  title,
  description,
  gradient,
  iconPaths,
}: SlideData) {
  return (
    <CarouselItem className="pl-0!">
      <div
        className={`relative h-40 sm:h-64 rounded-xl bg-linear-to-br ${gradient} flex items-center px-6 sm:px-12 overflow-hidden`}
      >
        <div className="absolute right-16 top-1/2 -translate-y-1/2 text-white/10">
          <SlideIcon title={label}>{iconPaths}</SlideIcon>
        </div>
        <div className="relative p-8">
          <p className="text-white/60 text-sm font-medium tracking-wider uppercase mb-2">
            {label}
          </p>
          <h2 className="text-white text-base sm:text-3xl font-bold mb-2">
            {title}
          </h2>
          <p className="text-white/70 text-sm sm:text-base">{description}</p>
        </div>
      </div>
    </CarouselItem>
  );
}

export function HomeCarousel() {
  return (
    <Carousel className="w-full">
      <CarouselContent>
        {SLIDES.map((slide) => (
          <CarouselSlide key={slide.label} {...slide} />
        ))}
      </CarouselContent>
      <CarouselPrevious className="absolute left-3 top-1/2 -translate-y-1/2 cursor-pointer" />
      <CarouselNext className="absolute right-3 top-1/2 -translate-y-1/2 cursor-pointer" />
    </Carousel>
  );
}
